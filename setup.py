...
if sys.argv[-1] in ['./setup.py','basic','build']:
  ...
  try:
    t=os.popen(complier+' util/cfitsio_test.f -o junk -L/usr/local/cfitsio -lcfitsio -fno-range-check -finit-local-zero -fno-automatic').read()
    if 'g' not in t:
          ...
          for name in files:
             try:
               if name.split('.')[0] == file and name.split('.')[1] == 'f': 
                 ...
                 options=options+' -L/usr/local/cfitsio -lcfitsio -fno-range-check -finit-local-zero -fno-automatic'
