...
def draw_x(usetex):
  global switch,axe,xmin,xmax,ymin,ymax,line,cmd,cid1,cid2,ft,master,ifile,box,pix
  global igray,r1,r2,c,ifit,label,dx,dy,f_label,filenames,ptype,lines,polly
...
# plot floating labels
  for z in f_label:
    if usetex:
      figtext((z[0]-xmin)/abs(xmax-xmin),(z[1]-ymin)/abs(ymax-ymin),z[-1], \
              horizontalalignment='center',verticalalignment='center',color=z[2],size=14)
    else:
      figtext((z[0]-xmin)/abs(xmax-xmin),(z[1]-ymin)/abs(ymax-ymin),z[-1], \
              horizontalalignment='center',verticalalignment='center',color=z[2],size=20)
...
# plot linear fits if they exist
  try:
    ...
    out=open('linfit.tmp','a')
    out.write(sys.argv[-1]+' '+str(b)+' '+str(a)+' '+str(bx)+' '+str(ax)+' '+str(bz)+' '+str(az)+'\n')
    out.close()
  except:
    pass
...
# draw manual lines
  for z in lines:
    if z[0] == 1.e33:
      axe.plot([z[1],z[1]],[ymin,ymax],z[3]+z[2])
    else:
...
# asymptotic fit
  ...
  if iasym:
    ...
    for tmp in arange(min(x)-delta,max(x)+delta,(max(x)-min(x))/200.):
      w.append(tmp) ; z.append(rat_calc(tmp,coeff,2))
    axe.plot(w,z,'b-')

    text(xmax-0.04*(xmax-xmin),rat_calc(x[-1],coeff,2),'$\leftarrow$',color='b',horizontalalignment='center',verticalalignment='center')
    mag1=rat_calc(w[-1],coeff,2)
    tmp='%6.3f' % (mag1)
    out=open('stds.tmp','w')
    out.write(str(master[0][0][-5])+' '+str(master[0][0][-4])+' '+tmp+'\n')
    out.close()
    figtext(0.14,0.85,'Asym mag =',color='b',horizontalalignment='left',verticalalignment='center')
    figtext(0.26,0.852,tmp,color='b',horizontalalignment='left',verticalalignment='center')

# draw boundaries for deletions
  try:
    polly
    last=polly[0]
    for z in polly[1:]:
      axe.plot([last[0],z[0]],[last[1],z[1]],'m-')
      last=z
    axe.set_xlim(xmin,xmax)
    axe.set_ylim(ymin,ymax)
  except:
...
def help():
  ...
  line=line+'r = reset                z = zoom border\n'
  ...
  line=line+'n = normalized gray    C = contour plot\n'
  line=line+'\n'
  ...
  line=line+'v = draw polygon for deletions'
  line=line+'\n'
...
#  global axe,xmin,xmax,ymin,ymax,line,cmd,cid1,cid2,ft,master,ifile,box,pix,igray,c,r1,r2,ifit,f_label,filenames,ptype
...
#  global xmin,xmax,ymin,ymax,line,cmd,cid1,cid2,ft,master,ifile,box,pix,igray,c,r1,r2,ifit,f_label,filenames,ptype
#
#  try:
#    pass
#  except:
...
def clicker(event): # primary event handler
  global switch,axe,xmin,xmax,ymin,ymax,line,cmd,cid1,cid2,ft,master,ifile,box,pix
  global igray,c,r1,r2,ifit,label,dx,dy,f_label,filenames,ptype,lines,polly
  global a,b,r,sigb,sigm,sig,ax,bx,rx,sigbx,sigmx,sigx,az,bz,delsig,sigma,iasym
...
#        if not tmp[4]:
#          filout1.write(tmp[-1])
#          filout1.write(' '.join([str(dum) for dum in tmp])+'\n')
#        else:
#          filout2.write(tmp[-1])
#          filout2.write(' '.join([str(dum) for dum in tmp])+'\n')
    filout1.close()
    filout2.close()
      ...
      for line in tmp.split('\n')[:-1]:
        if 'None' not in line:
          try:
            ...
            data=[]
            linex=[tmp.split() for tmp in open(line.split()[0],'r').readlines()]
            for z in linex:
              if len(z) < 1: continue
              try:
                if 'inf' in z[i] or 'inf' in z[j]: raise
                data.append([float(z[i]),float(z[j]),float(z[int(line.split()[8])]),
                             float(z[int(line.split()[9])]),0,z[-1],' '.join(z)])
              except:
                try:
                  if 'inf' in z[i] or 'inf' in z[j]: raise
                  data.append([float(z[i]),float(z[j]),0.,0.,0,z[-1],' '.join(z)])
                except:
                  pass
            master.append(data)
            ifile+=1
    ...
    if not polly:
      ...
      if ((polly[0][0]-event.xdata)**2.+(polly[0][1]-event.ydata)**2.)**0.5 < abs(xmin-xmax)/100.:
        tank=[]
        for i,data in enumerate(master):
          for n,tmp in enumerate(data):
            if point_inside_polygon(tmp[0],tmp[1],polly): data[n][4]=abs(data[n][4]-1)
          tank.append(data)
        master=tank
        polly=None
      else:
  ...
  elif event.key in ['g','n','N','C']: # build array for grayscale
      ...
      for jj,y in enumerate(arange(ymin,ymax,dy)):
        for ii,x in enumerate(arange(xmin,xmax,dx)):
          ...
          for t in master[ifit]:
            if t[0] >= min(x,x+dx) and t[0] < max(x,x+dx) and t[1] >= min(y,y+dy) and t[1] < max(y,y+dy): n+=1
    ...
    if event.key in ['n','N']:
      ...
      out.close()
#      print 'build_norm pick.tmp '+str(xmin)+' '+str(xmax)+' '+str(ymin)+' '+str(ymax)+' '+str(box)
      ...
      for jj,t in enumerate(tmp.split('\n')):
        for ii,n in enumerate(t.split()):
          if event.key == 'n':
            ...
            if float(n) != 0.:
              pix[jj][ii]=log10(float(n))
              if log10(float(n)) > r1: r1=log10(float(n))+2
            else:
              pix[jj][ii]=r2

    if event.key == 'C': # for a contour
      igray=-1
    ...
    igray=0 ; x=[] ; y=[] ; box=5. ; ifit=0 ; polly=None
    tank=[]
    for i,data in enumerate(master):
      for n,tmp in enumerate(data):
        x.append(tmp[0])
    ...
    if '-sfb' in sys.argv:
      tmp=ymin ; ymin=ymax ; ymax=tmp ; switch=1
...
#    draw_x(True)
#    fig.savefig('pick.ps')
    if 'prometheus' in os.uname()[1] and '-tex' not in sys.argv:
  ...
  elif event.key == 'z': # interactive set border
    dx=abs(xmax-xmin)/4.
    dy=abs(ymax-ymin)/4.
    xmin=event.xdata-dx
    xmax=event.xdata+dx
...
if '-sfb' in filenames: filenames.remove('-sfb')
for file in filenames:
  if file in ['-p','-b']: break
  ifile+=1
#  print 'loading ...',file,'('+colors[ifile]+')'
  try:
    linex=[tmp.split() for tmp in open(file,'r').readlines()]
  except:
  ...
  for z in linex:
    ...
    try:
      ...
      pass
    try:
      data.append([float(z[i]),float(z[j]),float(z[e1]),
                   float(z[e2]),0,z[-1],' '.join(z)])
    except:
      try:
        data.append([float(z[i]),float(z[j]),0.,0.,0,z[-1],' '.join(z)])
      except:
...
lines=[]                          # line marks
iasym=0                           # asymptotic fitting for curve of growth
...
if '-N' in sys.argv:
  box=int(box*2.**(int(sys.argv[sys.argv.index('-N')+1])))
  igray=1         # set grayscale switch
  dx=(xmax-xmin)/box
  dy=(ymax-ymin)/box
  pix=zeros((box,box),'Float32')
