...
def draw_h(usetex): # drawing routine, note the globals from clicker functions
  global xlow,xhi,ylow,yhi,xbin,line,colors,ft,cid1,cid2,raw,labels,cmd,norm,ax,scale,filnames,col
...
#  print 'cid2 key',event.key,'cmd',cmd,'line',line
  ...
  if event.key == 'enter': # <cr> disconnect, reconnect clicker, do action
    action()
    return 0
...
#  print 'in action',cmd
  ...
  if cmd in ['c']: # new column
    col=int(line.split(': ')[-1])
    raw=[]
    for files in filnames:
      x=[]
      input=[tmp.split() for tmp in open(files,'r').readlines()]
      for z in input:
        if len(z) < 1: continue
        try:
          if z[col] not in ['nan','inf']: x.append(float(z[col]))
        except:
          pass
      raw.append(x)
  ...
  if cmd in ['x','t']: # add labels
    ...
    try:
      ...
      input=[tmp.split() for tmp in open(line.split(': ')[-1],'r').readlines()]
      x=[]
      for z in input:
        if len(z) < 1: continue
        try:
...
def clicker(event): # event handler
  global xlow,xhi,ylow,yhi,xbin,line,colors,ft,cid1,cid2,raw,labels,cmd,norm,ax,scale,trap,filnames,col
#  print 'cid1 key',event.key,'cmd',cmd,'line',line,'trap',trap

  if event.key in ['x','t','f','m','c','F'] or trap: # note disconnect here and hand off to get_string
#                       but, not using get_string anymore cause disconnect fails in matplotlib
    if not trap:
      if event.key == 'x':
      ...
      if event.key in ['enter']: # <cr> disconnect, reconnect clicker, do action
        action()
        trap=0
      ...
      elif event.key in [None,'shift','control','alt','right','left','up','down','escape'] or ord(event.key) == 0: # ignore weird keys
        pass
