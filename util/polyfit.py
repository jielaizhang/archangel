...
if __name__ == '__main__':
  ...
  if '-h' in sys.argv:
    print './polyfit op order filename'
    print
    print 'fit an order polynomial to data in filename, first 2 columns'
    print 'options: -d = drop 100 points of fit over same range +/- 10%'
    print '         -e = drop 100 points of fit from x_o to x_n'
    print '         -r = drop fit in data x points'
    print '         -t = read fit from filename, drop y from x = order'
    print '              (if x filename, do all x)'
    sys.exit()

  if '-t' in sys.argv:
    a=[(map(float, tmp.split())) for tmp in open(sys.argv[-1],'r').readlines()][0]
    try:
      x=float(sys.argv[-2])
      out=0.
      for n,r in enumerate(a):
        out=out+r*x**(len(a)-n-1)
      print x,out
    except:
      data=[(map(float, tmp.split())) for tmp in open(sys.argv[-2],'r').readlines()]
      for x in data:
        out=0.
        for n,r in enumerate(a):
          out=out+r*x[0]**(len(a)-n-1)
        print x[0],out
  ...
  data=numarray.array([x,y])

  if '-d' in sys.argv or '-e' in sys.argv:
    a=polyfit(data,int(sys.argv[-2]))
    if '-e' in sys.argv:
      xmin=min(x)
      xmax=max(x)
    else:
    ...
    xstep=(xmax-xmin)/100.
    for t in arange(xmin,xmax+xstep,xstep):
#    dx=abs(x[0]-x[-1])
#    for t in arange(x[0]-2.50*dx,x[-1]+1.50*dx,(x[-1]-x[0])/200.):
      out=0.
      for n,r in enumerate(a):
        out=out+r*t**(len(a)-n-1)
      print t,out
  elif '-r' in sys.argv:
    a=polyfit(data,int(sys.argv[-2]))
    for t in x:
    ...
    print polyfit(data,int(sys.argv[-2]))
