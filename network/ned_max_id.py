...
if __name__ == "__main__":
  ...
  if sys.argv[1] == '-h':
    print '''
Usage: ned_max_ids master_file

seaches names in master file, output original name, NGC, IC, UGC in order
'''
    sys.exit()
  ...
  for file in files:
    ...
    for x in elements['RESOURCE'][0][2]['TABLE']:
      for z in x[2]['DATA'][0][2]['TABLEDATA'][0][2]['TR']:
        if ':' in z[2]['TD'][0][1]: continue
        if 'NGC' in z[2]['TD'][0][1] or \
           'IC' in z[2]['TD'][0][1] or \
           'LSBC' in z[2]['TD'][0][1] or \
           'UGC' in z[2]['TD'][0][1]:
          print z[2]['TD'][0][1].replace(' ','').replace('LSBC',''),
    print
