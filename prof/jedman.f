*     ELLIPSE FITTING
*
* this is a multi-revised verison of Jedrzejewski's original ellipse fitting program
* which is derived from Cawson's GASP PROF package - original programming is in CAPS -
* original comments lead with C

* this version contains:

* cfitsio routines, some iteration subraction
* advanced versus of this program has been rename prof.f
* in honor of the old prof.vf

C
C  THIS PROGRAM FITS ELLIPSES TO I*2 FRAMES OF ELLIPTICAL GALAXIES
C
	real PIX(4096,4096)
      real xdata(4096*4096)
      real inner(36),hold(18),output(100,18)
      character file1*100,file2*100,op*2,dp
	...
	REAL PIXVAL(1000),ANGLE(1000),MODEL(1000),INIT(18),WT(20),last(18)
	...
	REAL OUT(18), ONE, FA, MINSAMP, DATA(18), C(2), S(2), B4(20), B5(20)
...
C  INITIALISE VARIOUS CONTROL PARAMETERS
      ...
      call getarg(2,file1)
      call getarg(3,file2)
      if((op.eq.'-h').or.(op(1:1).ne.'-')
     *   .or.(file1=='').or.(file2=='')) then
        print*, ' '
        print '(a)', 'Usage: jedsub option file_name output_file'
        print*, ' '
        print '(a)', 'Options: -h = this mesage'
        print '(a)', '         -v = output each iteration'
        print '(a)', '         -q = quiet'
        goto 999
      endif
...
* open python parameter file, get pixel file name, read pixels, 
* initialize fit parameters, file format is

*101. 100.           : x and y centers (pixel units)
*1.                  : eccentricity (intial guess)
*0.                  : position angle (intial guess)
*-0.5                : shallowest slope for posn determination (MINSLOPE)
*1.1                 : ratio between successive major axes (MAJFACT)
*1.0                 : min distance for prof operation (DMIN)
*1000.               : max distance for prof operation (RMAX)
*8                   : minimum number of iterations (MINIT)
*20                  : maximum number of iterations (MAXIT)
*0.3                 : minimum fraction of ellipse inside frame (DELFACT)
*0.04                : maximum residual factor (RESTRSH)
*0.0                 : weight for undetermined points (UNDWT)
*5                   : starting sampling radius (start)
*n                   : dump ASCII file?
...
C  DO AN ITERATION
      ...
      xxx=best(1)+sig*RMSRES
	CALL NEWIT(PIX,NX,NY,DATA,MAJFACT,ANGLE,PIXVAL,xxx,jter)
      if(maxit.eq.1) then
	  CALL ANALYSE (ANGLE, PIXVAL, UNDWT, B, STDEV, NUM)
        xxx=b(1)+sig*RMSRES
	  CALL NEWIT(PIX,NX,NY,DATA,MAJFACT,ANGLE,PIXVAL,xxx,99)
      endif
 400	CONTINUE
...
C  NOT ENOUGH SAMPLED POINTS TO ATTEMPT A FIT
	...
	IF (NUM .LT. MINSAMP) THEN
      ...
      xxx=best(1)+sig*RMSRES
      CALL NEWIT(PIX,NX,NY,last,MAJFACT,ANGLE,PIXVAL,xxx,jter)
      CALL ANALYSE (ANGLE, PIXVAL, UNDWT, B, STDEV, NUM)
      LINOUT = LINOUT - 1
...
C  WRITE OUT THE INFO FOR THIS ITERATION IN DEMO MODE
      ...
      do j=1,ny
        do i=1,nx
          if(pix(i,j).ne.pix(i,j)) nerase=nerase+1
        enddo
      enddo
       if(op.eq.'-v') then
	WRITE (*,991) ITER,B(1),RMSRES,SLOPE,RAD,X0,Y0,ECC,POSANG,num,nerase
       endif
991	FORMAT(I4, F11.3, F8.2, F10.1,f7.2,2F7.2, F6.3, F7.2,3i5)
992	FORMAT(I4,'>', F10.3, F8.2, F10.1,f7.2,2F7.2, F6.3, F7.2,3i5)
993	FORMAT(I4,'*', F10.3, F8.2, F10.1,f7.2,2F7.2, F6.3, F7.2,3i5)
...
C  IF SLOPE IS TOO SMALL ON THE OUTSIDE OF THE GALAXY
	    ...
	    IF (SLOPE .GT. MINSLOPE*RMSRES) THEN
		IF (XBAR .GT. 0) THEN
		    X0 = XBAR
		    Y0 = YBAR
		END IF
       if(op.eq.'-v') then
		print *, 'Keeping ellipse parameters constant now'
	      print*, ' '
       endif
		MAXIT = 1
		MINIT = 1
...
C  IF MAXIMUM NUMBER OF ITERATION HAVE BEEN PERFORMED
	...
	IF (XITER .GE. MAXIT) THEN
     ...
     &	    BEST(16),BEST(13),BEST(14),num,nerase
	print*, ' '
	print*, '   #    Inten      RMS   Slope     Rad    XC   ',
     *         '  YC    Ecc    PA'
	print*, ' '
...
C  THATS IT, NOW ORDER THE PROFILES FILE IN ORDER OF INCREASING RADIUS
 ...
 999	continue
      ...
      call dump(output,nfile,file2,dp)
...
C  NEWIT IN PROGRAM RJNPROF.  OUTPUTS THE ARRAY B
	...
	REAL ANGLE(1000), PIXVAL(1000), C(2), S(2)
	REAL MEANINT
...
C  ARRAY WEIGHT.   NPTS POINTS, RETURNED SLOPE A AND INTERCEPT B
 ...
 999	RETURN
	...
	REAL HARMON(2),ANGLE(1000), PIXVAL(1000)
	...
	SUBROUTINE INTERP2(DATA,NX,NY,NBX,NBY,X,Y,DIAG,VALUE,AREA,BAD,xint,iter)
...
C  CROSSING POINT #R IS N(R,J,I)
	...
	LOGICAL DIAG, LBAD
	INTEGER NCROS(60,60),NIN(60,60),NCORN(60,60),NUMC(60,60)
...
C  INITILISE VARIABLES
		...
		IF ( .NOT. LBAD) THEN
                if(data(ix,iy).ge.xint.and.iter.ge.10) then
                  pixval=sqrt(-1.)
...
C  THIS SUBROUTINE DOES AN ITERATION IN RJNPROF
	...
	SUBROUTINE NEWIT(PIX,NX,NY,DATA,MAJFACT,ANGLE,PIXVAL,xint,iter)
	real PIX (4096,4096)
	INTEGER NX, NY, NBX, NBY, NSAMP, NANN, iter
	REAL DATA(18), ANGLE(1000), PIXVAL(1000), C(2), S(2)
...
C  AND CALL THE MORE COMPLICATED INTERPOLATION SUBROUTINE FOR R>20
	    ...
	    CALL INTERP2(PIX,NX,NY,NBX,NBY,XC,YC,.FALSE.,VALUE,AREA,BAD,xint,iter)
...
100       continue
	    APB = AREA + BAD
	    IF (APB .NE. 0.0) THEN
		IF (BAD/APB .GT. 0.05) PIXVAL(I) = sqrt(-1.)
	    END IF
 200	END DO
...
C  WELL THATS IT - WASNT TOO BAD WAS IT?
 ...
 200	    END DO
	...
	END
      ...
      subroutine dump(output,nfile,file,dp)
	real output(100,18),out(100)
      real MAJFACT,RESTRSH,X0,Y0,MINSLOPE
      character file*100,dp
      integer status,unit,readwrite,blocksize,nfound
      integer pc,gc,varidat
     ...
     *     '','','','','',''/
      ...
      if(dp.eq.'n'.or.dp.eq.'N') then
        status=0
        call ftgiou(unit,status)
        call ftopen(unit,file,1,blocksize,status)
        if(status.eq.0) then
          call ftdelt(unit,status)
        elseif (status.eq.103) then
          status=0
        ...
        call ftfiou(unit,status)
        call ftgiou(unit,status)
        call ftinit(unit,file,1,status)
        call ftphpr(unit,.true.,8,0,0,0,1,.true.,status)

        ifields=18
        extname='ELLIPSE_BINARY'
        varidat=0
        call ftcrhd(unit,status)
        call ftphbn(unit,nfile,ifields,ttype,tform,tunit,
     ...
     *            'Erase Sigma (JEDSUB)',status)
        ...
        do i=1,18
          ...
          do j=1,nfile
            ...
            xmin=1.e99
          enddo
          call ftpcle(unit,i,1,1,nfile,out,status)
        enddo
        call ftclos(unit,status)
        call ftfiou(unit,status)
      else
        ...
        do j=1,nfile
          ...
          enddo
          xold=xmin
          write(1,'(18(1pe12.4))') (output(iout,i),i=1,18)
        enddo
        close(unit=1)
      endif
