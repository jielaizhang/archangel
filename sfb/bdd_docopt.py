...
"""bdd_docopt.py - quick surface photometry calibration and fitting GUI. Re-work of bdd.py.
...
Options:
    ...
    -p              Force sfb rebuild [default: False]
    -a              Auto sfb build [default: False]
    -w              Use internal sky marks [default: False]
    -e              Start in r^1/4 mode [default: False]
    ...
    --errsig        Not sure what this does

Examples:
    python ~/src/DragonflyAnalysis/general/bin_images.py input.fits
"""
...
if __name__ == '__main__':
    ...
    sfbforcerebuild = arguments['-p']
    sfbautorebuild  = arguments['-a']
    internalskymarks= arguments['-w']
    rquatermode     = arguments['-e']
    sersicmode      = arguments['-s']
    ...
    option_c        = arguments['-c']
    option_errsig   = arguments['--errsig']
...
#XXXX    sys.exit()
    ...
    if os.path.exists(xmlfile):
        ...
        print xmlfile,'XML file not found - aborting'
        sys.exit()
...
#    manager.window.geometry('+400+50')
    ...
    if sfbforcerebuild or not isfb:
        ymax=sky+30.*skysig
        ymin=sky-10.*skysig
    else:
        ymin=min(s)-0.10*(max(s)-min(s))
