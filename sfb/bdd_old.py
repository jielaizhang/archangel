...
def airy(x,a):
  if len(a) == 4: # r^1/4 bulge + exp disk fit
...
#   program would like some first guess to speed things up
  ...
  dela=[0.1,0.1,0.1,]
  edge=[[5.,35.],[.5,5000.],[0.1,25.]] # set edges of fit
  nit=0
...
def fitx(npts,r,s,se_bulge,re_bulge,sstore,cstore,nt):
    ...
    if event.key == '/':
      ...
      for t in data:
        if t[1] - sky > 0 and t[2]: 
          ...
          if origin == '2MASS' and t[0]*xscale < 2.:
      ...
      if ifit in [3,4]:
        ...
        if ifit == 3:
          if str(re_bulge) == 'nan':
            ...
            se_bulge=cstore
          chisq_bulge,se_bulge,re_bulge,cstore,sstore=fitx(len(r),r,s,se_bulge,re_bulge,sstore,cstore,3)
        else:
          chisq_bulge,se_bulge,re_bulge,cstore,sstore=fitx(len(r),r,s,se_bulge,re_bulge,sstore,cstore,4)
