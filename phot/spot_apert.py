...
if sys.argv[1] == '-h':
  print 'spot_apert fits_file'
  print
  print 'reads x,y,aperture - calculates local sky'
  sys.exit()
...
while 1:
  ...
  rr=float(line.split()[2])

  left=max(0,int(xc-(rr+20)-5))
  right=min(int(xc+(rr+20)+5),nx)
  bottom=max(0,int(yc-(rr+20)-5))
  top=min(int(yc+(rr+20)+5),ny)
  sky=[]

  for x in numarray.arange(left,right,1):
    for y in numarray.arange(bottom,top,1):
      area=0.
      tmp=inside(x,y,1.,rr+10.,0.,xc,yc)
      tmp2=inside(x,y,1.,rr+15.,0.,xc,yc)
      if not tmp and tmp2 and str(pix[y-1][x-1]) != 'nan': sky.append(pix[y-1][x-1])
