...
# initialize edges, set write to zero
  ...
  std=open('/Users/js/stds/qphot.stds')
  stds=[]
  while 1:
    line=std.readline()
    if not line: break
    line.split()[-7].split(':')
    t1=float(line.split()[-7].split(':')[0])+float(line.split()[-7].split(':')[1])/60.+float(line.split()[-7].split(':')[2])/3600.
    t1=360.*t1/24.
    t2=abs(float(line.split()[-6].split(':')[0]))+float(line.split()[-6].split(':')[1])/60.+float(line.split()[-6].split(':')[2])/3600.
    if '-' in line.split()[-6].split(':')[0]: t2=-t2
    stds.append([line[:11].rstrip().replace(' ','_'),t1,t2])
  std.close()
...
#        pgptxt((nx/2.)-3600.*(std[1]-ra)/plate,(ny/2.)-3600.*(std[2]-dec)/plate,0.,0.5,std[0])
	  pgptxt((nx/2.)-3600.*(std[2]-dec)/plate,(ny/2.)-3600.*(std[1]-ra)/plate,0.,1.0,std[0])
    pgsch(1.0)
    pgsci(1)
