...
def plot_asym():
  ...
  global mag1,mag2,mag3,x1,x2,x3,err1,err2,err3,iasym,sky,half,half_r,fit_max,fit_min
...
#    print xmn,istop,imin
  ...
  if os.path.exists(root+'/'+name.split('.')[0]+'.fits'):
    igray=1
  else:
    igray=0
...
if '-h' in sys.argv:
  ...
  print 'X = write fit, flag as tot_mag_quality = fail'
  sys.exit()
...
def get_string(event): # event routine to get strings
  global axe,xmin,xmax,ymin,ymax,data,cmd,line,ft,cid1,cid2,imin,pix,r1,r2,prf,igray
  global mag1,mag2,mag3,x1,x2,x3,err1,err2,err3,iasym,sky,half_half_r,fit_max,fit_min
...
def clicker(event):
  ...
  if event.key in ['X','/']:
    ...
    if event.key == 'X':
      os.system('xml_archangel -e '+sys.argv[-1].split('.')[0]+' tot_mag_quality fail')
    else:
...
#      if round(line[3],2) == round((10.**(data[imin][0]))/scale,2): break
  ...
  if event.key == 's': line='Enter sky ('+str(sky)+'): '
