...
if len(sys.argv) == 1 or sys.argv[1] == '-h':
  ...
  print '                i = info on target closest to cursor'
...
while 1:
  ...
  if d[2] == 'i':
    if scan:
      rmin=1.e33
      for t in scan.split('\n'):
        if not len(t.split()): continue
        dd=((float(t.split()[0])-float(d[0]))**2+(float(t.split()[1])-float(d[1]))**2)**0.5
        if dd < rmin:
          rmin=dd
          out=t
      print out
  ...
  if d[2] == 's' or xsig == 0:
    if xsig == 0:
      ...
      mid=j1+(j2-j1)/2.
      xsig=xsig+2.*(float(d[0])-mid)/(j2-j1)
    pgeras()
    pgswin(i1,i2,j1,j2)
    pggray_s(pix[j1+1:j2+1,i1+1:i2+1],r1,r2,i1,j1,i2,j2)
    pgswin(i1+1.5,i2+1.5,j1+1.5,j2+1.5)
    ...
    tmp='%.2f' % (xsig*sig)
    scan=os.popen('gasp_images -f '+sys.argv[1]+' '+str(sky)+' '+tmp+' 2 false').read()
    print
    print 'gasp_images -f '+sys.argv[1]+' '+str(sky)+' '+tmp+' 2 false',
    pgsci(4)
    rmin=1.e33
    print ': ***',len(scan.split('\n'))-1,'targets found ***'
    print 
    for ell in scan.split('\n'):
      if len(ell) == 0: break
#      if len(scan.split('\n')) < 20: print ell
    ...
    pgsci(1)
    strng=str(len(scan.split('\n'))-1)+' targets: \gs = '+'%.1f' % xsig
